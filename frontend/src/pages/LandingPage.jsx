      ...
      <section id="program" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Pilih Program Sesuai Usia
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto">
              Program latihan terstruktur yang disesuaikan dengan tahap perkembangan anak
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-8">
            {programs.map((program) => (
              <Card key={program.id} className="overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-300 group">
                <div className="h-2 bg-gradient-to-r from-balida-blue via-balida-green to-balida-gold"></div>
                <CardContent className="p-8">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <Badge className="bg-balida-blue text-white mb-3">{program.category}</Badge>
                      <h3 className="text-2xl font-bold text-gray-900">{program.name}</h3>
                    </div>
                    <div className="text-right">
                      <p className="text-2xl font-bold text-balida-gold">{program.price}</p>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-4 mb-6">
                    <div className="flex items-center gap-2 text-gray-600">
                      <Calendar className="w-5 h-5 text-balida-blue" />
                      <span>{program.schedule}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <Clock className="w-5 h-5 text-balida-blue" />
                      <span>{program.time}</span>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <p className="font-semibold text-gray-900">Fokus Latihan:</p>
                    <div className="grid grid-cols-2 gap-2">
                      {program.focus.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <CheckCircle className="w-4 h-4 text-balida-green flex-shrink-0" />
                  ...
                  <Button 
                    onClick={() => window.open(ssbInfo.googleFormLink, '_blank')}
                    className="w-full mt-6 bg-balida-blue hover:bg-balida-blue/90 text-white font-semibold group-hover:bg-balida-gold transition-colors"
                  >
                    Daftar Program Ini
